					...
					<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
						{SERVICES.map((service, index) => (
							<Reveal key={service.title} delay={index * 0.06}>
                                <DepthSurface className="h-full" surfaceClassName="rounded-2xl" strength={5}>
								<a
									href={WHATSAPP_URL}
									target="_blank"
									rel="noopener noreferrer"
									className="depth-service-card group flex h-full flex-col rounded-2xl border border-border bg-card p-6 transition-all duration-300 hover:border-foreground/20 hover:shadow-[0_24px_48px_-24px_rgba(20,20,30,0.35)]"
								>
